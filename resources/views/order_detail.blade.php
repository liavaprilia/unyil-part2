<x-app-layout>
    ...
    <div x-data="orderDetails()" class="min-h-screen" id="order-details">
        ...
        <div class="p-6 mx-auto max-w-7xl">
            <div class="grid gap-8 lg:grid-cols-3">
                <!-- Main Content -->
                <div class="space-y-6 lg:col-span-2" id="main-content">
                    <!-- Order Confirmation (only show when status is shipped) -->
                    ...
                    <div class="bg-white border border-black">
                        ...
                        <form class="grid gap-6 p-6 md:grid-cols-1 base-form"
                            action="{{ route('order.confirm', $transaction->id_transaction) }}" method="POST"
                            data-function-callback="afterConfirmOrder" data-need-confirmation="true"
                            data-options-confirm='{
                                    "title": "Apakah Anda yakin pesanan sudah diterima?",
                            ...
                            <div class="p-4 border border-yellow-300 rounded bg-yellow-50">
                                <p class="text-sm font-bold text-yellow-800">
                                    Pesanan Anda sudah dikirim! Jika Anda sudah menerima pesanan, silakan konfirmasi
                            ...
                            <button type="submit"
                                class="flex items-center justify-center gap-2 px-6 py-3 text-sm font-bold text-white bg-green-600 border border-green-600 hover:bg-green-700">
                    ...
                    <!-- Resi & Foto Resi -->
                    @if ($transaction->tracking_number || $transaction->tshipping_proof)
                    <div class="bg-white border border-black">
                        <div class="p-4 border-b border-black">
                            <h2 class="text-xl font-bold">Resi & Foto Resi</h2>
                        </div>

                        <div class="grid gap-6 p-6 md:grid-cols-1">
                            <!-- Tracking Number -->
                            <div class="form-group">
                                <label class="block mb-3 text-sm font-bold text-gray-600 uppercase">NOMOR
                                    RESI</label>
                                <div class="flex gap-2">
                                    <input type="text" x-model="trackingNumber" placeholder="MASUKKAN NOMOR RESI"
                                        value="{{ $transaction->tshipping_tracking_number }}"
                                        name="tshipping_tracking_number" disabled readonly
                            ...
                            <div class="col-span-2 space-y-4 form-group">
                                <div class="px-4 py-4 mb-4">
                                    ...
                                    <div @click="showReceiptModal = true"
                        ...
                        <div class="p-4">
                            <div class="space-y-3">
                                ...
                                {{-- <div class="flex flex-wrap gap-2">
                                    <span
                                        class="px-3 py-2 text-sm font-bold bg-gray-100 border border-black">{{ $transaction->tshipping_provience }}</span>
                                <span
                                    class="px-3 py-2 text-sm font-bold bg-gray-100 border border-black">{{ $transaction->tshipping_city }}</span>
                                <span
                                    class="px-3 py-2 text-sm font-bold bg-gray-100 border border-black">{{ $transaction->tshipping_district }}</span>
                                <span
                                    class="px-3 py-2 text-sm font-bold bg-gray-100 border border-black">{{ $transaction->tshipping_subdistrict }}</span>
                                <span
                                    class="px-3 py-2 text-sm font-bold bg-gray-100 border border-black">{{ $transaction->tshipping_zip_code }}</span>
                            </div> --}}
                            @php
                            $alamat = collect([
    ...
    <div x-show="showReceiptModal" x-transition:enter="transition ease-out duration-300"
        ...
        <div @click.stop class="max-w-2xl max-h-screen overflow-auto bg-white border-4 border-black">
            ...
            <div class="p-6">
                <div class="flex items-center justify-center mb-6 bg-gray-100 border border-gray-300 h-96">
                    <img :src="receiptSrc" alt="Bukti Pembayaran"
                        class="object-contain w-full h-full rounded" />
                </div>
                {{-- <div class="p-4 border border-gray-300 bg-gray-50">
    ...
    <script>
        function orderDetails() {
            return {
                ...
                receiptSrc: '{{ $transaction->tshipping_proof ? asset('storage/' . $transaction->tshipping_proof) : 'https://placehold.co/700x320' }}',
        ...
        function afterConfirmOrder(response) {
